...
try {
  ...
  & $binaryPath uninstall --remove-path --purge --yes
  ...
  if ([IO.File]::ReadAllText($profilePath) -ne "before`r`nafter`r`n") { throw 'standalone CLI E2E: PATH block was not removed' }
} finally {
